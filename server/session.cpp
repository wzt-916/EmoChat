...
json Session::recvMsg()
{
    ...
    while (1)
    {
        int res = recv(m_socket, buffer, 4, 0); //读取4字节数据
        if (res > 0)
            break;
        if (res <= 0)
        {
    ...
    memcpy(&len, buffer, 4); // 使用memcpy从字节转换为整数， 很明显 每个json字符串前还有4个字节 告诉接收端这次json的长度

    char *msg = new char[len + 1];
    memset(msg, 0, len + 1);            
    recv(m_socket, msg, len, MSG_WAITALL); //读取json数据
